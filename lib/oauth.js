...
exports.v1 = function (settings) {
    ...
    return async function (request, h) {
        ...
        if (request.query.error === 'access_denied' ||
            ...
            return internals.unauthenticated(h, Boom.internal('Application rejected'), { credentials });
        }
        ...
        if (protocol !== 'https' &&
            ...
            return internals.unauthenticated(h, Boom.internal('Invalid setting  - isSecure must be set to false for non-https server'), { credentials });
        }
        ...
        if (!request.query.oauth_token) {
            ...
            try {
                ...
                return internals.unauthenticated(h, err, { credentials });
            }
        ...
        if (!request.query.oauth_verifier) {
            return internals.unauthenticated(h, Boom.internal('Missing verifier parameter in ' + name + ' authorization response'), { credentials });
        }
        ...
        if (request.query.oauth_token !== state.token) {
            return internals.unauthenticated(h, Boom.internal(name + ' authorized request token mismatch'), { credentials });
        }
        ...
        if (!settings.provider.profile ||
            ...
            return internals.authenticated(h, { credentials });
        }
...
exports.v2 = function (settings) {
    ...
    return async function (request, h) {
        ...
        if (request.query.error === 'access_denied' ||
            ...
            return internals.unauthenticated(h, Boom.internal(`App rejected: ${request.query.error_description || request.query.denied || 'No information provided'}`), { credentials });
        }
        ...
        if (state.nonce !== requestState.substr(0, Math.min(requestState.length, internals.nonceLength))) {
            return internals.unauthenticated(h, Boom.internal('Incorrect ' + name + ' state parameter'), { credentials });
        }
        ...
        try {
            ...
            return internals.unauthenticated(h, Boom.internal('Failed obtaining ' + name + ' access token', err), { credentials });
        }

        if (tokenRes.statusCode < 200 ||
            tokenRes.statusCode > 299) {

            return internals.unauthenticated(h, Boom.internal('Failed obtaining ' + name + ' access token', payload), { credentials });
        }
        ...
        try {
            ...
            return internals.unauthenticated(h, Boom.internal('Received invalid payload from ' + name + ' access token endpoint', payload), { credentials });
        }
        ...
        if (!settings.provider.profile || settings.skipProfile) {
            return internals.authenticated(h, { credentials, artifacts: payload });
        }
...
internals.authenticated = function (h, credentials) {

    return h.authenticated ? h.authenticated(credentials) : new Promise.resolve(credentials);
};

internals.unauthenticated = function (h, error, credentials) {

    return h.unauthenticated ? h.unauthenticated(error, credentials) : new Promise.reject({ error, credentials });
};
...
internals.refreshRedirect = function (request, name, protocol, settings, credentials, h) {
    ...
    if (request.query.refresh) {
        return internals.unauthenticated(h, Boom.internal('Missing ' + name + ' request token cookie'), { credentials });
    }
