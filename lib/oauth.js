...
exports.v2 = function (settings) {
    ...
    return function (request, reply) {
        ...
        if (!request.query.code) {
            ...
            query.response_type = settings.provider.response_type;
            query.redirect_uri = internals.location(request, protocol, settings.location);
            query.state = nonce;
            if (settings.provider.nonce) {
                query.nonce = nonce; // required by microsoftonline
            }
            
            const responseMode = settings.provider.response_mode;
            if (responseMode) {
        ...
        if (settings.provider.resource) {
            query.resource = settings.provider.resource
        }
