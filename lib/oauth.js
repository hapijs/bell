...
exports.v2 = function (settings) {
    ...
    return async function (request, h) {
        ...
        if (!request.query.code) {
            ...
            h.state(cookie, state);
            var resp =  await h.redirect(`${settings.provider.auth}?${internals.queryString(query)}`);
            return h.response(resp).takeover();
        }
        ...
        try {
            ...
            var res = await Wreck.post(settings.provider.token, requestOptions);
            const tokenRes = res.res;
            const tokenPayload = res.payload;
        ...
        const get = async function (uri, params) {
            ...
            try {
                const prof_res = await Wreck[settings.provider.profileMethod](uri + getQuery, getOptions);
                const profileRes = prof_res.res;
                var profilePayload = prof_res.payload;
