...
internals.authenticated = function (h, credentials) {
    ...
    return h.authenticated ? h.authenticated(credentials) : credentials;
};

internals.unauthenticated = function (h, error, credentials) {

    error.reformat({ credentials });
    return h.unauthenticated ? h.unauthenticated(error, credentials) : Promise.reject(error);
};
