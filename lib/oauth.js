...
exports.v1 = function (settings) {
    ...
    return async function (request, h) {
        ...
        const get = async function (uri, params) {
            ...
            const profile = await client.resource('get', uri, params, { token: payload.oauth_token, secret: payload.oauth_token_secret });
...
exports.v2 = function (settings) {
    ...
    return async function (request, h) {
        ...
        let payload = null;
        try {
            const { res: tokenRes, payload: tokenPayload } = await Wreck.post(settings.provider.token, requestOptions);
        ...
        const get = async function (uri, params) {
            ...
            try {
                const { res: profileRes, payload: resPayload } = await Wreck[settings.provider.profileMethod](uri + getQuery, getOptions);
                if (profileRes.statusCode !== 200) {
                    throw Boom.internal(`Failed obtaining ${name} user profile`, { payload: profilePayload, statusCode: profileRes.statusCOde });
                }

                const profilePayload = internals.parse(resPayload);
...
internals.refreshRedirect = function (request, name, protocol, settings, h) {
    ...
    try {
        ...
        if (!options.raw) {
            ...
            if (parsedPayload instanceof Error) {
                throw Boom.internal('Received invalid payload from ' + this.provider + ' ' + desc + ' endpoint', parsedPayload);
            }
        }
