    ...
    return function (request, reply) {
        ...
        Wreck.post(settings.provider.token, requestOptions, function (err, res, payload) {
            ...
            var get = function (uri, params, callback) {
                ...
                var query = (params ? '?' + internals.queryString(params) : '');
...
internals.Client.prototype._request = function (method, uri, params, oauth, options, callback) {
    ...
    Wreck[method](uri, requestOptions, function (err, res, payload) {
        ...
        if (err ||
            ...
            return callback(Boom.internal('Failed obtaining ' + self.provider + ' ' + desc, err || payload));
        }

        payload = internals.parse(payload);
        if (payload instanceof Error) {
            return callback(Boom.internal('Received invalid payload from ' + self.provider + ' ' + desc + ' endpoint', payload));
        }
