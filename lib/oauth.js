...
exports.v1 = function (settings) {
    ...
    return async function (request, h) {
        ...
        // Replace h.authenticated and h.unauthenticated if not invoked via a strategy

        if (!h.authenticated){
            h.authenticated = (credentials) => Promise.resolve(credentials);
        }

        if (!h.unauthenticated) {
            h.unauthenticated = (error, credentials) => Promise.reject({ credentials, ...error });
        }
