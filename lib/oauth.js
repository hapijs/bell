    ...
    return function (request, reply) {
        ...
        if (!request.query.oauth_token) {
            ...
            return client.temporary(oauth_callback, (err, payload) => {
                ...
                return reply.redirect(settings.provider.auth + '?' + internals.queryString(authQuery, settings.escapeScope));
            });
        }
...
exports.v2 = function (settings) {
    ...
    return function (request, reply) {
        ...
        if (!request.query.code) {
            ...
            reply.state(cookie, state);
            return reply.redirect(settings.provider.auth + '?' + internals.queryString(query, settings.escapeScope));
        }
        ...
        const requestOptions = {
            payload: internals.queryString(query, settings.escapeScope),
            headers: {
        ...
        Wreck.post(settings.provider.token, requestOptions, (err, tokenRes, payload) => {
            ...
            const get = function (uri, params, callback) {
                ...
                const getQuery = (params ? '?' + internals.queryString(params, settings.escapeScope) : '');
...
internals.refreshRedirect = function (request, name, protocol, settings, reply) {
    ...
    const refreshUrl = internals.location(request, protocol, settings.location) + '?' + internals.queryString(refreshQuery, settings.escapeScope);
...
internals.Client.prototype._request = function (method, uri, params, oauth, options, callback) {
    ...
    if (params) {
        const paramsString = internals.queryString(params, this.settings.escapeScope);
        if (method === 'get') {
            uri += '?' + paramsString;
        }
        else {
...
internals.Client.queryString = internals.queryString = function (params, escapeScope) {
    ...
    for (let i = 0; i < keys.length; ++i) {
        ...
        if (Array.isArray(value)) {
            for (let j = 0; j < value.length; ++j) {
                fields.push(internals.qsPair(ks, key, value, escapeScope, j));
            }
        }
        else {
            fields.push(internals.qsPair(ks, key, value, escapeScope));
        }
    }
...
internals.qsPair = (ks, key, value, escapeScope, ix) => {

    const val = ix > -1 ? value[ix] : value;
    let pair = ks;
    pair += (key === 'scope' && !escapeScope) ? val : internals.encodePrimitive(val);
