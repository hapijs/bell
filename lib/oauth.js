...
exports.v2 = function (settings) {
    ...
    return async function (request, h) {
        ...
        try {
            ...
            const oAuthError = err.data.headers['www-authenticate'];
            let errorMessage = `Failed obtaining ${name} access token`;

            if (oAuthError) {
                errorMessage += `\n\nService responded with: ${oAuthError}`;
            }

            return h.unauthenticated(Boom.internal(errorMessage, err), { credentials });
        }
