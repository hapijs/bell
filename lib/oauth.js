...
exports.v1 = function (settings) {
    ...
    return async function (request, h) {
        ...
        client.settings.temporary = client.settings.temporary ||
          internals.Client.baseUri(internals.resolveProviderEndpoint(request, settings.provider.temporary));
        client.settings.token = client.settings.token ||
          internals.Client.baseUri(internals.resolveProviderEndpoint(request, settings.provider.token));
...
exports.Client = internals.Client = function (options) {
    ...
    const temporary = internals.resolveProviderEndpoint(null, options.provider.temporary);
    const token = internals.resolveProviderEndpoint(null, options.provider.token);
