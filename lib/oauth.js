...
exports.v1 = function (settings) {
    ...
    return async function (request, h) {
        ...
        if (query.error === 'access_denied' ||
            ...
            throw Boom.internal('Application rejected');
        }

        // If not https but cookie is secure, throw error

        if (protocol !== 'https' && settings.isSecure) {
            throw Boom.internal('Invalid setting  - isSecure must be set to false for non-https server');
        }
        ...
        if (!request.query.oauth_verifier) {
            throw Boom.internal('Missing verifier parameter in ' + name + ' authorization response');
        }

        const state = request.state[cookie];
        if (!state) {
            return h.response(internals.refreshRedirect(request, name, protocol, settings)).takeover();
        }

        h.unstate(cookie);

        if (request.query.oauth_token !== state.token) {
            throw Boom.internal(name + ' authorized request token mismatch');
        }
        ...
        const get = async (uri, params = {}) => {
            ...
            const { payload: response } = await client.resource('get', uri, params, { token: payload.oauth_token, secret: payload.oauth_token_secret });
            return response;
        };
...
exports.v2 = function (settings) {
    ...
    return async function (request, h) {
        ...
        if (request.query.error === 'access_denied' ||
            ...
            throw Boom.internal(`App rejected: ${request.query.error_description || request.query.denied || 'No information provided'}`);
        }
        ...
        if (state.nonce !== requestState.substr(0, Math.min(requestState.length, internals.nonceLength))) {
            throw Boom.internal('Incorrect ' + name + ' state parameter');
        }
        ...
        try {
            ...
            throw Boom.internal('Failed obtaining ' + name + ' access token', err);
        }

        if (tokenRes.statusCode < 200 ||
            tokenRes.statusCode > 299) {

            throw Boom.internal('Failed obtaining ' + name + ' access token', payload);
        }
        ...
        try {
            ...
            throw Boom.internal('Received invalid payload from ' + name + ' access token endpoint', payload);
        }
...
internals.parse = function (payload) {
    ...
    if (payload.trim()[0] === '{') {
        try {
            ...
            throw new Boom.internal('Invalid JSON payload', err);       // Convert JSON errors to application errors
        }
    }
