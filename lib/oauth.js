...
exports.v2 = function (request, reply) {
    ...
    if (!request.query.code) {
        ...
        query.redirect_uri = request.server.location(this.settings.path, request);
        query.state = nonce;
    ...
    var query = {
        ...
        redirect_uri: request.server.location(this.settings.path, request)
    };
    ...
    if (this.settings.provider.headers) {
        Hoek.merge(requestOptions.headers, this.settings.provider.headers);
    }
...
internals.Client.prototype.request = function (method, uri, params, oauth, tokenSecret, type, callback) {
    ...
    Nipple[method](uri + query, { headers: { Authorization: internals.Client.header(oauth) } }, function (err, res, payload) {
