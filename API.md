...
- `escapeScope` - by default scopes will be url escaped, however, this is incompatible with some providers (`google`) which have scopes that are full secure urls (e.g. `https://www.googleapis.com/auth/admin.directory.user.readonly`). When escaped, these scopes cause an unrecognized scope error from the provider. Set `escapeScope` to false to prevent scope escape and preserve valid format. Default to `true`.
